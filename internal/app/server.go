...
import (
	...
	"github.com/cg219/locster/internal/database"
	"github.com/cg219/locster/pkg/argon2id"
	"github.com/cg219/locster/pkg/webtoken"
...
func addRoutes(srv *Server) {
    ...
    srv.mux.Handle("GET /assets/", http.StripPrefix("/assets", http.FileServer(http.Dir("static-app/assets"))))
...
func (s *Server) setTokens(w http.ResponseWriter, r *http.Request, username string) {
    ...
    accessToken.Create("locster")
    refreshToken.Create("locster")
    cookieValue := webtoken.CookieAuthValue{ AccessToken: accessToken.Value(), RefreshToken: refreshToken.Value() }
    cookie := webtoken.NewAuthCookie("locster", "/", cookieValue, int(time.Hour * 24 * 30))
...
func (s *Server) unsetTokens(w http.ResponseWriter, r *http.Request) {
    ...
    cookie := webtoken.NewAuthCookie("locster", "/", webtoken.CookieAuthValue{}, int(0))
...
func (s *Server) getAuthGookie(r *http.Request) (string, string) {
    cookie, err := r.Cookie("locster")
    if err != nil {
        s.log.Error("Cookie Retrieval", "cookie", "locster", "method", "UserOnly", "request", r, "error", err.Error())
        return "", ""
    }
...
func (s* Server) refreshAccessToken(ctx context.Context, refreshExpire int64, refreshTokenString, refreshValue, username string, w http.ResponseWriter) {
    ...
    cookie := webtoken.NewAuthCookie("locster", "/", cookieValue, int(refreshExpire))
